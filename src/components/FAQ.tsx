        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => {
            ...
            return (
              <div
                key={faq.question}
                className="rounded-2xl border border-border/70 bg-background/40 transition"
              >
                <button
                  onClick={() => toggleQuestion(index)}
                  className="w-full px-5 py-4 text-left flex items-center justify-between gap-4 transition hover:bg-background/60"
                >
                  <h3 className="text-lg font-semibold text-textPrimary pr-4">{faq.question}</h3>
                  <span
                    className={`flex-shrink-0 text-textMuted transition-transform duration-200 ${
                      isOpen ? 'rotate-180' : ''
                    }`}
                  >
                    ↓
