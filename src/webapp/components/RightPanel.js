...
const RightPanel = () => {
    ...
    return (_jsxs("aside", { className: "sticky top-20 hidden xl:flex w-80 flex-col gap-5", children: [_jsxs("div", { className: `rounded-2xl p-5 ${theme === 'dark' ? 'border border-darkBorder bg-darkBgElevated/50' : 'bg-backgroundElevated shadow-sm'}`, children: [_jsx("label", { className: `mb-3 block text-xs font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-darkTextMuted' : 'text-textMuted'}`, children: "Search Kural" }), _jsxs("div", { className: "relative", children: [_jsx("input", { type: "text", value: query, onChange: (e) => setQuery(e.target.value), placeholder: "Search topics or people", className: `w-full rounded-xl px-4 py-2.5 text-sm outline-none transition-all duration-200 focus:ring-2 focus:ring-accent/20 ${theme === 'dark'
                                ...
                                : theme === 'dark'
                                    ...
                                    : 'bg-backgroundSubtle hover:bg-backgroundHover text-textPrimary'}`, children: item.topic }, item.topic))) }))] }), _jsxs("div", { className: `rounded-2xl p-5 ${theme === 'dark' ? 'border border-darkBorder bg-darkBgElevated/50' : 'bg-backgroundElevated shadow-sm'}`, children: [_jsx("h3", { className: `mb-4 text-sm font-bold ${theme === 'dark' ? 'text-darkTextPrimary' : 'text-textPrimary'}`, children: currentUser?.interests && currentUser.interests.length > 0
                            ? 'People with similar interests'
                            : 'People to follow' }), isLoadingSuggestions ? (_jsx("div", { className: "py-4 text-center", children: _jsx("p", { className: `text-xs ${theme === 'dark' ? 'text-darkTextMuted' : 'text-textMuted'}`, children: "Loading suggestions..." }) })) : peopleToFollow.length === 0 ? (_jsx("div", { className: "py-4 text-center", children: _jsx("p", { className: `text-xs ${theme === 'dark' ? 'text-darkTextMuted' : 'text-textMuted'}`, children: "No suggestions yet" }) })) : (_jsx("div", { className: "space-y-3", children: peopleToFollow.map((person) => {
                            ...
                            const personInitials = person.name
                                ...
                                .join('')
                                .slice(0, 2);
                            return (_jsxs("div", { className: `rounded-xl px-3.5 py-3.5 transition-all duration-200 ${theme === 'dark' ? 'bg-white/5 hover:bg-white/10' : 'bg-backgroundSubtle hover:bg-backgroundHover'}`, children: [_jsxs("div", { className: "flex items-start justify-between mb-2", children: [_jsxs("div", { className: "flex items-center gap-3 flex-1 min-w-0", children: [_jsx(Link, { to: `/app/profile/${person.id}`, className: `flex-shrink-0 w-10 h-10 rounded-full overflow-hidden bg-primary/20 flex items-center justify-center transition-colors ${theme === 'dark' ? '' : ''}`, children: person.profilePictureUrl ? (_jsx("img", { src: person.profilePictureUrl, alt: `${person.name}'s profile`, className: "w-full h-full object-cover" })) : (_jsx("span", { className: "text-xs font-semibold text-primary", children: personInitials })) }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx(Link, { to: `/app/profile/${person.id}`, className: `block text-sm font-bold truncate hover:text-accent transition-colors ${theme === 'dark' ? 'text-darkTextPrimary' : 'text-textPrimary'}`, children: person.name }), _jsxs(Link, { to: `/app/profile/${person.id}`, className: `block text-xs truncate hover:text-accent transition-colors ${theme === 'dark' ? 'text-darkTextMuted' : 'text-textMuted'}`, children: ["@", person.handle] }), matchingInterests.length > 0 && (_jsxs("div", { className: "mt-2 flex flex-wrap gap-1.5", children: [matchingInterests.slice(0, 2).map((interest, idx) => (_jsx("span", { className: `px-2 py-0.5 text-[10px] font-semibold bg-accent/20 text-accent rounded-md ${theme === 'dark' ? 'border border-accent/30' : ''}`, children: interest }, idx))), matchingInterests.length > 2 && (_jsxs("span", { className: `px-2 py-0.5 text-[10px] ${theme === 'dark' ? 'text-darkTextMuted' : 'text-textMuted'}`, children: ["+", matchingInterests.length - 2, " more"] }))] }))] })] }), _jsx("button", { onClick: () => (following ? unfollowUser(person.id) : followUser(person.id)), className: `ml-3 rounded-lg px-3.5 py-1.5 text-xs font-semibold transition-all duration-200 whitespace-nowrap active:scale-95 ${following
                                                    ? theme === 'dark' ? 'bg-white/10 text-darkTextMuted' : 'bg-backgroundHover text-textMuted'
                                                    : 'bg-gradient-to-r from-primary to-accent text-white hover:from-primaryHover hover:to-accentHover shadow-sm hover:shadow-md'}`, children: following ? 'Following' : 'Follow' })] }), overlapCount > 0 && (_jsxs("p", { className: `text-[10px] mt-1.5 ${theme === 'dark' ? 'text-darkTextMuted' : 'text-textMuted'}`, children: [overlapCount, " shared interest", overlapCount !== 1 ? 's' : ''] }))] }, person.id));
                        }) }))] })] }));
};
