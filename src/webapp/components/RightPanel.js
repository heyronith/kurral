...
const RightPanel = () => {
    ...
    return (_jsxs("aside", { className: "sticky top-20 hidden xl:flex w-80 flex-col gap-5", children: [_jsxs("div", { className: "rounded-2xl border-2 border-border/60 bg-card/60 p-5 shadow-card backdrop-blur-md", children: [_jsx("label", { className: "mb-3 block text-xs font-semibold uppercase tracking-wide text-textMuted", children: "Search Kurral" }), _jsxs("div", { className: "relative", children: [_jsx("input", { type: "text", value: query, onChange: (e) => setQuery(e.target.value), placeholder: "Search topics or people", className: "input-field" }), _jsx("span", { className: "absolute right-3 top-2.5 text-xs text-textMuted font-medium", children: "\u2318 K" })] })] }), _jsx(TrendingNewsSection, {}), _jsxs("div", { className: "rounded-2xl border-2 border-border/60 bg-card/60 p-5 shadow-card backdrop-blur-md", children: [_jsx("h3", { className: "mb-4 text-sm font-bold text-textPrimary", children: "Trending Topics" }), topicsLoading ? (_jsx("div", { className: "py-4 text-center", children: _jsx("p", { className: "text-xs text-textMuted", children: "Loading trending topics..." }) })) : displayTrendingTopics.length === 0 ? (_jsx("div", { className: "py-4 text-center", children: _jsx("p", { className: "text-xs text-textMuted", children: "No trending topics yet" }) })) : (_jsx("div", { className: "flex flex-wrap gap-2", children: displayTrendingTopics.map((item) => (_jsx("button", { onClick: () => selectTopic(item.topic), className: `inline-flex items-center justify-center rounded-lg px-3 py-1.5 text-sm font-medium text-textPrimary border transition-all duration-200 hover:bg-backgroundElevated/40 active:scale-95 cursor-pointer ${item.matchesInterest
                                ? 'border-accent/40 bg-accent/10 hover:bg-accent/15'
                                : 'border-border/60 bg-transparent hover:border-border/80'}`, children: item.topic }, item.topic))) }))] }), _jsxs("div", { className: "rounded-2xl border-2 border-border/60 bg-card/60 p-5 shadow-card backdrop-blur-md", children: [_jsx("h3", { className: "mb-4 text-sm font-bold text-textPrimary", children: currentUser?.interests && currentUser.interests.length > 0
                            ? 'People with similar interests'
                            : 'People to follow' }), isLoadingSuggestions ? (_jsx("div", { className: "py-4 text-center", children: _jsx("p", { className: "text-xs text-textMuted", children: "Loading suggestions..." }) })) : peopleToFollow.length === 0 ? (_jsx("div", { className: "py-4 text-center", children: _jsx("p", { className: "text-xs text-textMuted", children: "No suggestions yet" }) })) : (_jsx("div", { className: "space-y-3", children: peopleToFollow.map((person) => {
                            ...
                            const personInitials = person.name
                                ...
                                .join('')
                                .slice(0, 2);
                            return (_jsxs("div", { className: "rounded-xl px-3.5 py-3.5 border border-border/50 hover:bg-backgroundElevated/60 transition-all duration-200 hover:border-border/70", children: [_jsxs("div", { className: "flex items-start justify-between mb-2", children: [_jsxs("div", { className: "flex items-center gap-3 flex-1 min-w-0", children: [_jsx(Link, { to: `/profile/${person.id}`, className: "flex-shrink-0 w-10 h-10 rounded-full overflow-hidden border-2 border-border bg-primary/20 flex items-center justify-center hover:border-accent transition-colors", children: person.profilePictureUrl ? (_jsx("img", { src: person.profilePictureUrl, alt: `${person.name}'s profile`, className: "w-full h-full object-cover" })) : (_jsx("span", { className: "text-xs font-semibold text-primary", children: personInitials })) }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx(Link, { to: `/profile/${person.id}`, className: "block text-sm font-bold text-textPrimary truncate hover:text-accent transition-colors", children: person.name }), _jsxs(Link, { to: `/profile/${person.id}`, className: "block text-xs text-textMuted truncate hover:text-accent transition-colors", children: ["@", person.handle] }), matchingInterests.length > 0 && (_jsxs("div", { className: "mt-2 flex flex-wrap gap-1.5", children: [matchingInterests.slice(0, 2).map((interest, idx) => (_jsx("span", { className: "px-2 py-0.5 text-[10px] font-semibold bg-accent/20 text-accent rounded-md border border-accent/30", children: interest }, idx))), matchingInterests.length > 2 && (_jsxs("span", { className: "px-2 py-0.5 text-[10px] text-textMuted", children: ["+", matchingInterests.length - 2, " more"] }))] }))] })] }), _jsx("button", { onClick: () => (following ? unfollowUser(person.id) : followUser(person.id)), className: `ml-3 rounded-lg px-3.5 py-1.5 text-xs font-semibold transition-all duration-200 whitespace-nowrap active:scale-95 ${following
                                                    ? 'bg-backgroundElevated/80 text-textMuted border border-border/60 shadow-subtle'
                                                    : 'bg-gradient-to-r from-primary to-accent text-white hover:from-primaryHover hover:to-accentHover shadow-button hover:shadow-buttonHover'}`, children: following ? 'Following' : 'Follow' })] }), overlapCount > 0 && (_jsxs("p", { className: "text-[10px] text-textMuted mt-1.5", children: [overlapCount, " shared interest", overlapCount !== 1 ? 's' : ''] }))] }, person.id));
                        }) }))] })] }));
};
