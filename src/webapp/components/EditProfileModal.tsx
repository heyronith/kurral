      ...
      <div className={`relative w-full max-w-xl max-h-[90vh] overflow-y-auto rounded-2xl border p-5 shadow-2xl ${theme === 'dark' ? 'border-white/20 bg-black' : 'border-border bg-background/95'}`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-4 pt-4 border-t border-border/50">
            <h3 className={`text-lg font-semibold ${theme === 'dark' ? 'text-white' : 'text-textPrimary'}`}>
              Account Settings
            </h3>
            
            <div className={`p-6 rounded-lg border-2 ${theme === 'dark' ? 'border-red-500/30 bg-red-500/5' : 'border-red-200 bg-red-50'}`}>
              <h4 className={`text-base font-semibold mb-2 ${theme === 'dark' ? 'text-white' : 'text-red-900'}`}>
                Delete Account
              </h4>
              <p className={`text-sm mb-2 ${theme === 'dark' ? 'text-white/80' : 'text-red-800'}`}>
