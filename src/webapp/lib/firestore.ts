    ...
    try {
      ...
      try {
        ...
        const mentionQuery = query(
          ...
          limit(500) // Firestore limit, process in batches if needed
        );
        ...
        if (!mentionSnapshot.empty) {
          ...
          mentionSnapshot.docs.forEach((chirpDoc) => {
            ...
            const updatedMentions = mentions.filter((id: string) => id !== userId);
      ...
      try {
        let lastMentionCommentDoc: any = null;
        ...
        while (hasMoreMentionComments) {
          ...
          if (lastMentionCommentDoc) {
            mentionQuery = query(
              ...
              orderBy('createdAt', 'desc'),
              startAfter(lastMentionCommentDoc),
              limit(500)
            );
          } else {
            mentionQuery = query(
        ...
        try {
          ...
          for (let i = 0; i < deletedChirpIds.length; i += chunkSize) {
            ...
            const bookmarkQuery = query(
              ...
              limit(500) // Firestore limit, but we'll handle pagination manually if needed
            );
