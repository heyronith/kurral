...
const firebaseConfig = {
  ...
  messagingSenderId:
    readEnv('VITE_FIREBASE_MESSAGING_SENDER_ID') || readEnv('FIREBASE_MESSAGING_SENDER_ID') || '',
...
if (!isConfigValid) {
  ...
  if (isBrowser) {
    (window as any).firebaseError = new Error(errorMessage);
  }
}
...
try {
  ...
  if (isBrowser) {
    (window as any).firebaseError = error;
  }
  throw error;
}
