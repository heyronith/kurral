...
const inferLegacyTopicFromText = (text) => {
...
const extractFallbackTopics = (text, limit = 6) => {
    const tokens = text
        .toLowerCase()
        .match(/[a-z0-9#]{3,}/g) || [];
...
const buildContentAnalysisInstruction = (availableTopicNames, availableTopicDetails) => {
...
export class ReachAgent {
     ...
     */
    async suggestTopicsAndReach(text, availableTopics, userTopics) {
        try {
...
- Content intent and target audience`;
            const result = await this.agent.generateJSON(prompt, SYSTEM_INSTRUCTION, {
                type: 'object',
                properties: {
                    suggestedTopics: {
                        type: 'array',
     ...
     */
    async suggestReachSettings(text, topic, userTopics) {
        try {
...
let reachAgentInstance = null;
export const getReachAgent = () => {
    if (!BaseAgent.isAvailable()) {
        return null;
    }
...
export const suggestReachSettingsFallback = (text, topic) => {
    const lowerText = text.toLowerCase();
    const isDiscussionPrompt = lowerText.includes('?') ||
        lowerText.includes('what do you think') ||
        lowerText.includes('thoughts') ||
