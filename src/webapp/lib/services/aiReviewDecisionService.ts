 ...
 */
export async function decideFinalStatus(
  chirpId: string,
  consensus: ConsensusSnapshot
): Promise<'clean' | 'blocked' | 'needs_review'> {
  try {
    const chirp: Chirp | null = await chirpService.getChirp(chirpId);
    if (!chirp) {
  ...
  // Baseline ratios
  const validateRatio =
    consensus.totalWeight > 0 ? consensus.validateWeight / consensus.totalWeight : 0;
  const invalidateRatio =
    consensus.totalWeight > 0 ? consensus.invalidateWeight / consensus.totalWeight : 0;

  // Fact-check signals (optional)
  const factChecks = chirp.factChecks || [];
  const hasFalseHighConfidence = factChecks.some(
    (fc) => fc.verdict === 'false' && (fc.confidence ?? 0) > 0.7
  );
