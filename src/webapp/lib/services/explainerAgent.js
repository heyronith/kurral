...
const fallbackExplanation = (valueScore, claims, factChecks, discussion) => {
    const parts = [
...
export async function generateValueExplanation(chirp, valueScore, claims, factChecks, discussion) {
...
- Return JSON with a single "summary" field.`;
    try {
        const response = await agent.generateJSON(prompt, 'Value explanation writer', EXPLANATION_SCHEMA);
        return response.summary.trim();
    }
    catch (error) {
        console.error('[ExplainerAgent] Error generating explanation:', error);
        return fallbackExplanation(valueScore, claims, factChecks, discussion);
    }
}
