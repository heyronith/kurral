 ...
 */
async function ensureValueStatsInitialized(userId) {
    try {
...
export async function recalculateAllActiveUsers() {
    try {
        const thirtyDaysAgo = new Date(Date.now() - THIRTY_DAYS_MS);
        const sinceTimestamp = Timestamp.fromDate(thirtyDaysAgo);
        const q = query(CONTRIBUTIONS_COLLECTION, where('createdAt', '>=', sinceTimestamp));
        const snapshot = await getDocs(q);
        ...
        const results = await Promise.allSettled(Array.from(userIds).map((userId) => recalculateUserReputation(userId)));
...
export function startPeriodicRecalculation(intervalMs = 24 * 60 * 60 * 1000) {
