...
export async function processChirpValue(chirp) {
    try {
        ...
        const valueScore = await withRetry(() => scoreChirpValue(chirp, claims, factChecks, discussion), 'value scoring');
        const explanation = await withRetry(() => generateValueExplanation(chirp, valueScore, claims, factChecks, discussion.threadQuality), 'explanation generation');
        ...
        if (Object.keys(insights).length > 0) {
            await withRetry(() => chirpService.updateChirpInsights(chirp.id, insights), 'updating chirp insights');
        }
...
export async function processCommentValue(comment) {
    try {
        ...
        await withRetry(() => chirpService.updateChirpInsights(chirp.id, {
            valueScore,
            valueExplanation: explanation,
            discussionQuality: discussion.threadQuality,
            factCheckStatus: policyDecision.status,
