...
import { collection, query, where, orderBy, limit, getDocs, addDoc, doc, getDoc, setDoc, updateDoc, Timestamp, writeBatch, onSnapshot, } from 'firebase/firestore';
...
export const notificationService = {
     ...
     */
    async tryAggregateNotification(notificationData) {
        try {
            const now = Date.now();
            const windowStart = now - AGGREGATION_WINDOW_MS;
            // Find unread notifications of same type in aggregation window
            const q = query(collection(db, 'notifications'), where('userId', '==', notificationData.userId), where('type', '==', notificationData.type), where('read', '==', false), where('dismissed', '==', false), orderBy('createdAt', 'desc'), limit(10));
