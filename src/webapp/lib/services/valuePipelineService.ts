...
import { preCheckChirp, preCheckComment, preCheckText, type PreCheckResult } from './factCheckPreCheckAgent';
...
function matchClaimsToOriginal(
  ...
  const matches = new Map<string, { originalClaim: Claim; originalFactCheck?: FactCheck; similarity: number }>();
...
export async function processChirpValue(
  ...
  if (chirp.rechirpOfId) {
    ...
    preCheckResult = await safeExecute('pre-check', () =>
      withRetry(() => preCheckChirp(chirp), 'pre-check')
    );
    shouldProceedWithFactCheck = preCheckResult?.needsFactCheck ?? false;
  }
  ...
  let claimMatches: Map<string, { originalClaim: Claim; originalFactCheck?: FactCheck; similarity: number }> | undefined;
    ...
    if (!factChecksResult || factChecksResult.length === 0) {
      ...
      if (chirp.quotedChirpId && quotedChirp && hasCompleteFactCheckData(quotedChirp) && claimMatches && claimMatches.size > 0) {
        ...
        if (claimsToCheck.length > 0) {
          newFactChecks = await safeExecute('fact checking (new claims only)', () =>
