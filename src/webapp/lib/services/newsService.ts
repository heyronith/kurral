// News Service - Generates news from platform trending topics using AI
import type { Chirp, TrendingNews } from '../../types';
...
import { discoverStories, type StoryCluster } from './storyDiscoveryAgent';
...
const generationLocks = new Map<string, Promise<TrendingNews[]>>();
...
const createStorySignature = (story: StoryCluster): string => {
...
const mapPostsById = (posts: Chirp[]): Map<string, Chirp> =>
...
const ensureMinimumTopics = (topics: string[], fallback: string[]): string[] => {
  if (topics.length > 0) {
    return topics;
  }
  return fallback;
...
const sanitizeTopics = (topics: string[]): string[] =>
...
const getTrendingTopicNames = async (limit: number = 5): Promise<string[]> => {
  try {
...
const resolveTopicsForUser = async (userId?: string | null): Promise<{ legacyTopics: string[]; semanticInterests: string[] }> => {
  if (userId) {
    try {
...
export const newsService = {
  ...
  async _generateNewsInternal(scopedUserId: string, userId: string | null | undefined, forceRefresh: boolean): Promise<TrendingNews[]> {
    try {
      ...
      const engagementCount =
        storyPosts.reduce((sum, post) => sum + post.commentCount, 0) || storyPosts.length;
  ...
  async getCachedNews(scopedUserId: string = GLOBAL_USER_ID): Promise<TrendingNews[]> {
    try {
      const q = query(
  ...
  async checkExistingNewsBySignature(storySignature: string, scopedUserId: string): Promise<TrendingNews | null> {
    try {
      const now = Date.now();
      ...
      try {
        const q = query(
          collection(db, 'trendingNews'),
          where('userId', '==', scopedUserId),
          where('storySignature', '==', storySignature),
          orderBy('lastUpdated', 'desc'),
          limit(1)
        );
        const snapshot = await getDocs(q);
  ...
  async cleanupOldNews(scopedUserId: string = GLOBAL_USER_ID): Promise<void> {
    try {
      const q = query(
        collection(db, 'trendingNews'),
        where('userId', '==', scopedUserId),
  ...
  async clearCache(scopedUserId: string = GLOBAL_USER_ID): Promise<void> {
    try {
      const snapshot = await getDocs(
        query(collection(db, 'trendingNews'), where('userId', '==', scopedUserId))
      );
      const deletePromises = snapshot.docs.map(doc => 
        setDoc(doc.ref, { _deleted: true }, { merge: true })
      );
  ...
  async updateEngagementCount(newsId: string, increment: number = 1): Promise<void> {
    try {
      const newsRef = doc(db, 'trendingNews', newsId);
      const newsSnap = await getDoc(newsRef);
      if (newsSnap.exists()) {
