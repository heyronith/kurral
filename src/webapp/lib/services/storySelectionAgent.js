...
export function selectBestStory(stories, existingNews, userId) {
    if (stories.length === 0) {
    ...
    existingNews.forEach((news) => {
        ...
        if (!signature)
            return;
        const isRecent = now - news.lastUpdated.getTime() < STORY_DUPLICATE_WINDOW_MS &&
    ...
    const scoredStories = stories
        ...
        const score = newsworthiness * 0.5 +
            confidence * 0.25 +
            engagementBonus * 0.2 +
            diversityBonus -
            duplicatePenalty;
    ...
    const reason = `Highest combined score (${scoredStories[0].score.toFixed(2)}) based on newsworthiness, confidence, and engagement`;
    return {
