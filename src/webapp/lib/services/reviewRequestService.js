...
const calculatePriority = (chirp, user) => {
    ...
    if (chirp.semanticTopics && user.semanticTopics) {
        const matchingTopics = chirp.semanticTopics.filter((topic) => user.semanticTopics?.some((userTopic) => userTopic.toLowerCase().includes(topic.toLowerCase()) ||
...
export const reviewRequestService = {
    async getPendingReviewRequests(userId) {
        try {
            ...
            const q = query(collection(db, 'chirps'), orderBy('createdAt', 'desc'), limit(100) // Fetch more than needed, then filter
            );
            const snapshot = await getDocs(q);
            const allChirps = snapshot.docs.map(chirpFromFirestore);
            // Filter chirps that need review
            const chirpsNeedingReview = allChirps.filter((chirp) => chirp.factCheckStatus === 'needs_review');
