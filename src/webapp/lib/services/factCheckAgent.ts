...
const ensureEvidenceQuality = (evidence: FactCheckResponse['evidence'], isWebSearch: boolean = false): FactCheck['evidence'] => {
...
const parseJsonFromSearchResponse = (response: any): FactCheckResponse | null => {
    ...
    if (jsonMatch) {
      try {
        ...
        if (parsed && typeof parsed === 'object' && 'verdict' in parsed) {
          return parsed as FactCheckResponse;
        }
      } catch (error) {
...
const runFactCheckWithWebSearch = async (chirp: Chirp, claim: Claim): Promise<FactCheck> => {
  ...
  if (response?.output && Array.isArray(response.output)) {
    for (const outputBlock of response.output) {
      ...
      if ('content' in outputBlock && Array.isArray((outputBlock as any).content)) {
        for (const contentBlock of (outputBlock as any).content) {
      ...
      if ('tool_calls' in outputBlock && Array.isArray((outputBlock as any).tool_calls)) {
        const toolCalls = (outputBlock as any).tool_calls;
        if (toolCalls.some((tc: any) => tc?.type === 'web_search' || tc?.function?.name === 'web_search')) {
          responseDebug.hasWebSearchResults = true;
        }
      }
    }
