...
const buildArticleId = (raw: RawNewsArticle, suffix: string): string => {
  // Use article_id from newsdata.io or fallback to link + pubDate
  const base = raw.article_id || `${raw.link}::${raw.pubDate}`;
  return `${base}::${suffix}`;
};

const transformArticle = (raw: RawNewsArticle, metadata: { category?: string; query?: string; tag: string }): NewsArticle => {
...
const fetchTopHeadlines = async (config: CategoryConfig): Promise<NewsArticle[]> => {
  ...
  return (result.results || []).map((article: RawNewsArticle) =>
    transformArticle(article, { category: config.category, tag: `category:${config.category}` })
  );
};

const fetchEverything = async (config: QueryConfig): Promise<NewsArticle[]> => {
...
export const newsApiService = {
  isConfigured: (): boolean => ensureConfigured(),

  async fetchDiverseArticles(): Promise<NewsArticle[]> {
    if (!ensureConfigured()) {
      return [];
    }
