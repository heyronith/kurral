...
interface NewsSummary {
  ...
  summary: string; // Max 280 characters for list view
  fullDescription: string; // 2-3 paragraphs for detail view
  keyFacts: string[]; // Array of key facts extracted
  confidence: number; // 0-1 confidence score
  relatedTopics: string[]; // Topics that might be related
...
export async function generateNewsFromPosts(
  posts: Chirp[],
  topic: string,
...
Respond with ONLY a valid JSON object in this exact format:
{
  ...
  try {
    const response = await agent.generateJSON<NewsSummary>(prompt, systemInstruction);
