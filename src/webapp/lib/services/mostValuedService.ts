...
import type { Chirp, ValueScore, DiscussionQuality } from '../../types';

type Timeframe = 'today' | 'week' | 'month' | 'all';

export interface MostValuedQueryOptions {
  timeframe?: Timeframe;
  limit?: number;
  interests?: string[];
...
export interface MostValuedResult {
  posts: Chirp[];
...
const toValueScore = (raw: any, fallbackDate: Date): ValueScore | undefined => {
  ...
  return {
    ...
    drivers: Array.isArray(raw.drivers)
      ? raw.drivers.filter((driver: any) => typeof driver === 'string')
      : undefined,
  };
};

const toDiscussionQuality = (raw: any): DiscussionQuality | undefined => {
  if (!raw || typeof raw !== 'object') return undefined;
  return {
...
const mapDocToChirp = (doc: QueryDocumentSnapshot): Chirp => {
  const data = doc.data();
...
const getTimeframeStart = (timeframe: Timeframe): Date | null => {
  const now = Date.now();
  switch (timeframe) {
    case 'today':
...
const buildConstraints = (options: MostValuedQueryOptions) => {
...
const filterVisible = (snapshot: QuerySnapshot) => {
  const now = new Date();
...
export const mostValuedService = {
  ...
  async getValuedPosts(options: MostValuedQueryOptions = {}): Promise<MostValuedResult> {
