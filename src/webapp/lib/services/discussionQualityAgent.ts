...
import type { Chirp, Comment, DiscussionQuality, ValueVector } from '../../types';

type CommentInsight = {
  role: Comment['discussionRole'];
  contribution: ValueVector & { total: number };
};

export type DiscussionAnalysis = {
  threadQuality: DiscussionQuality;
  commentInsights: Record<string, CommentInsight>;
};
...
const normalizeVector = (vector: ValueVector & { total?: number }): ValueVector & { total: number } => {
  const safe = {
...
const fallbackAnalysis = (comments: Comment[]): DiscussionAnalysis => {
  ...
  const insights: Record<string, CommentInsight> = {};
...
const summarizeComment = (comment: Comment): string => {
  return `Comment ${comment.id} by ${comment.authorId}: ${comment.text.trim()}`.slice(0, 600);
};

export async function analyzeDiscussion(chirp: Chirp, comments: Comment[]): Promise<DiscussionAnalysis> {
  if (comments.length === 0) {
...
Analyze the discussion. Score each dimension from 0 (poor) to 1 (excellent). Return JSON per schema.`;
  ...
  try {
    const response = await agent.generateJSON<{
      threadQuality: DiscussionQuality;
      commentInsights: Array<{
        commentId: string;
        role: Comment['discussionRole'];
        contribution: ValueVector;
      }>;
    }>(prompt, SYSTEM_PROMPT, ANALYSIS_SCHEMA);

    const commentInsights: Record<string, CommentInsight> = {};
    response.commentInsights?.forEach((insight) => {
      if (!insight.commentId) return;
