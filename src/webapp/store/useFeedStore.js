...
const mergeChirpLists = (existing, incoming) => {
        ...
        try {
            ...
            set((state) => {
                ...
                return {
                    ...
                    chirps: state.chirps.map((c) => c.id === newComment.chirpId && shouldIncrementCount
            ...
            processCommentValue(newComment)
                ...
                set((state) => {
                    ...
                    const updatedCommentList = insight
                        ? chirpComments.map((comment) => comment.id === newComment.id
                            ? {
                                ...comment,
                    ...
                    return {
                        ...
                        chirps: result.updatedChirp
                            ? state.chirps.map((chirp) => chirp.id === result.updatedChirp?.id ? result.updatedChirp : chirp)
                            : state.chirps,
                    };
                });
            })
    ...
    deleteChirp: async (chirpId, authorId) => {
        try {
            ...
            set((state) => ({
                ...
                comments: Object.fromEntries(Object.entries(state.comments).filter(([id]) => id !== chirpId)),
            }));
        }
        catch (error) {
