...
export class InstructionService {
  ...
  constructor() {
    if (!BaseAgent.isAvailable()) {
      throw new Error(
        'AI agent is not available. Please configure OpenAI proxy to use instruction interpretation.'
      );
    }
  ...
  async interpretInstruction(
    ...
    try {
      ...
      throw new Error(
        error?.message || 'Failed to interpret instruction. Please check your OpenAI configuration and try again.'
      );
    }
  }
...
export const instructionService = (() => {
  if (!BaseAgent.isAvailable()) {
    console.error(
      'InstructionService cannot be initialized: OpenAI proxy is not configured. ' +
      'Please configure EXPO_PUBLIC_OPENAI_PROXY_URL to use instruction interpretation features.'
    );
